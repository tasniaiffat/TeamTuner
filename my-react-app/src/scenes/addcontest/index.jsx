...
const AddContest = () => {
  ...
  const [topItems, setTopItems] = useState([
    { id: 'cf-div3', title: 'Codeforces Div 3', date: '13 May, 2024', time: '8:30 p.m.' },
    { id: 'cf-div2', title: 'Codeforces Div 2', date: '14 May, 2024', time: '9:00 a.m.' },
  ]);

  const [bottomItems, setBottomItems] = useState([
    { id: 'cf-div1', title: 'Codeforces Div 1', date: '15 May, 2024', time: '6:00 p.m.' },
    { id: 'cf-div4', title: 'Codeforces Div 4', date: '16 May, 2024', time: '7:00 a.m.' },
  ]);
  ...
  const sortByDateTime = (items) =>
    items.sort((a, b) => parseDateTime(a.date, a.time) - parseDateTime(b.date, b.time));
  ...
  return (
    ...
    <Box m="20px" sx={{ bgcolor: colors.primary[400] }}>
      ...
      <nav aria-label="primary task folders">
        <List>
        <Typography ml = '18px' color={colors.greenAccent[500]} variant="h5" gutterBottom>
  Added Contests
</Typography>

          {topItems.map(item => (
            <ListItem key={item.id} disablePadding>
              <ListItemButton onClick={() => moveItem(item, topItems, bottomItems, setTopItems, setBottomItems)}>
                <ListItemIcon>
      ...
      <nav aria-label="secondary task folders">
        <List>
        <Typography ml = '18px' color={colors.redAccent[500]} variant="h5" gutterBottom>
  Removed Contests
</Typography>
          {bottomItems.map(item => (
            <ListItem key={item.id} disablePadding>
              <ListItemButton onClick={() => moveItem(item, bottomItems, topItems, setBottomItems, setTopItems)}>
                <ListItemIcon>
                  <CheckBoxOutlineBlankIcon style={{ color: colors.redAccent[500] }} />
                </ListItemIcon>
                <ListItemText primary={formatListItem(item)} />
              </ListItemButton>
            </ListItem>
  ...
  <IconButton onClick={() => setShowModal(true)} style={{ color: colors.greenAccent[400], position: 'fixed', bottom: 20, right: 20 }}>
    <AddIcon />
  </IconButton>
</Tooltip>
...
(
  ...
  >
    <DialogTitle><h3>Add New Contest</h3></DialogTitle>
    <DialogContent>
      <TextField
        autoFocus
        required
